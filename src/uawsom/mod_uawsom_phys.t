  ...
  subroutine uawsom_to_conserved_origin(ixI^L,ixO^L,w,x)
    ...
    if(uawsom_energy) then
      w(ixO^S,e_)=w(ixO^S,p_)*inv_gamma_1&
                 ...
                 +uawsom_mag_en(w, ixI^L, ixO^L)+uawsom_w_en(w, ixI^L, ixO^L)
...
!                      dlog(mach_high(ixO^S)/mach(ixO^S))/dlog(mach_high/mach_low))
  ...
  subroutine uawsom_get_flux(wC,w,x,ixI^L,ixO^L,idim,f)
    ...
    ! Get flux of energy
    ! f_i[e]=v_i*e+v_i*ptotal-b_i*(b_k*v_k) + Q-wplus + Q+wminus
    if(uawsom_energy) then
      if (uawsom_internal_e) then
    ...
    f(ixO^S,wminus_)= w(ixO^S,wminus_)*(w(ixO^S,mom(idim)) + w(ixO^S,mag(idim))&
                                 /(w(ixO^S,rho_)*(zeta(ixO^S)+1.d0)/2.d0)**0.5d0)

    ! compute flux of wplus
    f(ixO^S,wplus_)= w(ixO^S,wplus_)*(w(ixO^S,mom(idim)) - w(ixO^S,mag(idim))&
                                 /(w(ixO^S,rho_)*(zeta(ixO^S)+1.d0)/2.d0)**0.5d0)
  ...
  !> Compute wminus and wplus energy
  function uawsom_w_en(w, ixI^L, ixO^L) result(we)
    use mod_global_parameters, only: nw, ndim
    integer, intent(in)           :: ixI^L, ixO^L
    double precision, intent(in)  :: w(ixI^S, nw)
    double precision              :: we(ixO^S)

    we = w(ixO^S,wminus_) + w(ixO^S,wplus_)

  end function uawsom_w_en
